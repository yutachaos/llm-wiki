...
- lint: 全体整合性チェックを実施し3件を修正。(1) wiki/sources/basecamp-manager-playbook.md の出典リンクが `../sources/` で自己参照になっていたため `../../sources/`（生ソース）へ修正、(2) wiki/sources/karpathy-llm-wiki-gist.md に生ソースへのリンクがなかったため追加、(3) wiki/topics/platform-engineering.md の「## 矛盾」節の直後に概要の続き段落が孤立していたため概要内へ移動
- add: 既存ページが言及するのに定義ページがない概念、および複数トピックに登場するのに entity ページがない人物・組織を分析して補完。topics 6件を新規作成（westrum-organizational-culture: dora-metrics が引く「創造的文化」の出典 / learning-organization: センゲ・アージリス、知識経営の隣接理論 / okr: マネジメント実務の空白 / high-output-management: engineering-manager の委譲論の理論側 / motivation-theory: 評価・1on1 の前提 / sre-slo-error-budget: platform-engineering が前提に置く「信頼性の規律」）、entities 6件を新規作成（takeuchi-hirotaka / andy-grove / nicole-forsgren / camille-fournier / will-larson / cncf）。出典は各ページ冒頭に記載（ソースファイルなし、モデル知識由来）
- lint: CLAUDE.md の「関連ページは双方向にリンク」に反する片方向リンク（既存分11件＋新規分11件）を全て解消。37signals Manager Playbook 群が理論ページ群と無リンクで孤立していた点も、okr / motivation-theory / high-output-management / engineering-manager との相互リンクで接続。index.md の「マネジメント実務（37signals Manager Playbook）」節を「マネジメント実務」に改名し OKR・High Output Management を収容、index 更新
...
- lint: 全体整合性チェック＋日本語レビュー（natural-japanese）を実施。機械的チェック（リンク切れ・孤立ページ・幽霊エントリ・frontmatter・重複ページ）は問題なし。main からの merge で生じた片方向リンク8件を解消（nonaka-ikujiro ↔ peter-drucker、knowledge-worker ↔ managing-oneself / federal-decentralization / sources-of-innovation、effective-executive ↔ managing-oneself、manager-playbook-development ↔ managing-oneself、peter-drucker ↔ okr）。日本語2箇所を修正（manager-playbook-development の「核心的な」、ai-fluency-4d-framework の「〜ではなく」鋳型反復）。cncf.md・cncf-platforms-white-paper.md の年号（2015・2023）は sources/ 原文に存在しない数値のため未検証として報告のみ
